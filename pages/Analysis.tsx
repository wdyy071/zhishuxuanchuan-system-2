...
const Analysis: React.FC = () => {
  ...
  // Fallback Logic:
  // 1. Try hotspot
  // 2. Try Panorama Data (if clicked from Panorama view or non-hotspot list)
  let displayData = currentHotspot;
  
  if (!displayData) {
     const panoramaData = getMockPanoramaData();
     const found = panoramaData.find(p => p.id === id || p.code === id); // Handle ID or Code navigation
     if (found) {
        displayData = {
          id: found.id,
          name: found.name,
  ...
  // Final Fallback if ID is completely invalid
  if (!displayData) {
     displayData = {
      id: 'default',
      name: '半导体ETF',
  ...
  // Get dynamic competitors based on product code
  ...
  useEffect(() => {
    if (timeRange === '实时') {
      // Pass competitors to generator to ensure overlay data exists
      setChartData(generateChartData(competitors));
    } else {
      setChartData(generateHistoricalData(timeRange, competitors));
    }
  }, [displayData.code, timeRange]); // Depend on code to regenerate when switching products
  ...
  // Helper to get competitor data from the dynamic list
  ...
  const IntradayTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-slate-200 shadow-lg rounded-md text-xs z-50 min-w-[200px]">
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Toolbar */}
      <div className="flex justify-between items-center bg-white p-4 rounded-md border border-slate-100">
        ...
        <div className="flex items-center gap-3">
           <button 
             ...
             className="flex items-center gap-2 px-4 py-2 text-slate-600 hover:text-brand bg-slate-50 hover:bg-brand-light rounded transition-colors text-sm font-medium"
           >
          ...
          <button 
             ...
             className="flex items-center gap-2 px-4 py-2 bg-brand hover:bg-brand-dark text-white rounded transition-colors text-sm font-medium shadow-sm shadow-brand/30"
           >
      ...
      <div className="grid grid-cols-12 gap-6">
                ...
                <div className="px-6 pt-6 pb-2">
                  ...
                  <div className="flex items-start gap-12">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="font-mono text-3xl font-medium text-slate-500 tracking-tight">
                    ...
                    <AreaChart data={chartData} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
                      ...
                      {showCompetitor ? (
                        ...
                        <Area 
                          type="monotone" 
                          dataKey="value" 
                          stroke="#0e57b4" // Changed from Orange to Blue
                          strokeWidth={3}
                          fill="url(#colorHist)" 
                        />
                      )}
              ...
              <div className="flex border-t border-slate-200">
                {['实时', '近1月', '近3月', '近6月', '更多'].map(t => (
                    <button 
                      ...
                      className={`flex-1 py-3 text-sm transition-all relative flex items-center justify-center gap-1
                        ${timeRange === t 
                          ? 'text-brand font-bold bg-white' 
                    ...
                    >
                      ...
                      {timeRange === t && (
                        <span className="absolute bottom-0 left-0 w-full h-[3px] bg-brand"></span>
                      )}
                    </button>
                ))}
              </div>
        ...
        <div className="col-span-12 lg:col-span-5">
           <div className="bg-white p-6 rounded-md border border-slate-100 shadow-sm h-full">
             ...
             <div className="overflow-hidden rounded-md border border-slate-200 overflow-x-auto">
             ...
             <div className="mt-6 bg-brand-light p-4 rounded-md">
        ...
        <div className="grid grid-cols-2 gap-8">
          ...
          <div>
             ...
             <div className="space-y-2 h-[400px] overflow-y-auto custom-scrollbar pr-2">
                {newsList.map(item => (
                  <div key={item.id} className="flex items-start gap-2 text-sm group cursor-pointer hover:bg-slate-50 p-1 rounded-md">
